...
class fzf_locate_all(Command):
    ...
    def execute(self):
        import subprocess
        if self.quantifier:
            command="mdfind 'kind:file' -onlyin %s %s | fzf -e -i" % (os.getcwd(), self.arg(1))
        else:
            command="mdfind 'kind:file' -onlyin %s %s | fzf -e -i" % (os.getcwd(), self.arg(1))
        fzf = self.fm.execute_command(command, stdout=subprocess.PIPE)
        stdout, stderr = fzf.communicate()
...
class fzf_locate_dirs(Command):
    ...
    def execute(self):
        import subprocess
        if self.quantifier:
            command="mdfind 'kind:folder' -onlyin %s %s | fzf -e -i" % (os.getcwd(), self.arg(1))
        else:
            command="mdfind 'kind:folder' -onlyin %s %s | fzf -e -i" % (os.getcwd(), self.arg(1))
        fzf = self.fm.execute_command(command, stdout=subprocess.PIPE)
        stdout, stderr = fzf.communicate()
